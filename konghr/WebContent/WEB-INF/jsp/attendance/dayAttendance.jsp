...
$(document).ready(function(){
    ...
    function registDayAttd(clock,clockTime){
	    ...
	    convertTimePicker(); // 맴버변수 conversionDate에 timePicker 선택한 시간을 변환
		...
		if(clockCheck=="Clock"){		// 시계기록 버튼 클릭
		    dayAttd = {
				"empCode" : empCode,
			...
			success : function(data){
				...
				if(data.restAttdList){ // 가져온 데이터가 존재한다면
					...
					if(restTypeCode=='ADC003'){
						...
						restPublicApplyEndTime = data.restAttdList.endTime;
					}
    	...
    	if(clockCheck!="Clock"){
    		...
    		if(Number(conversionDate) < 1800 && $("#attdTypeCode").val() == 'ADC002'){ //선택 시간이 18시보다 빠르고 퇴근이라면
				...
				if(restTypeCode == 'ADC003'){ //금일 확정된 공외출 정보가 존재한다면
					alert(restPublicApplyEndTime+"시까지 들어와야합니다")
