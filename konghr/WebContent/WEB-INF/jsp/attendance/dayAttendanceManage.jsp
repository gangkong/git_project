...
var dayAttdMgtList = [];
    ...
    function finalizeDayAttdMgt(){
	    ...
	    $("#dayAttdMgtList_grid").find("tr:not(:first)").each(function(i, element) {	// tr의 첫번쨰 태그뺴고 전부다.
		    var dateData = $(this).find("td").eq(2).text();							// td태그의 3번쨰 태그.
		    dayAttdMonthData = dateData.substring(0,dateData.lastIndexOf("-"));
		    if(dateData.substring(5,6)==0) // 월에 0이 들어가 있다면   ex]2018-08
				dayAttdMonthData = replaceLast(dayAttdMonthData,0,"") // 0을 제거한다  ex]2018-8 
